...
const highlightEvent = {
    ...
    desc: 'The biggest technology conference in Southeast Asia. Join 5,000+ developers, designers, and entrepreneurs for 3 days of talks, workshops, and networking.',
    date: 'May 15–17, 2026',
    location: 'Jakarta Convention Center',
    attendees: 4832,
    status: 'Trending',
    image: 'https://picsum.photos/seed/dform-highlight-tech/1200/640',
};
</script>

<template>
    <section class="border-y-[1.5px] border-[var(--brutal-ink)] bg-[var(--brutal-yellow)]/10 py-14">
            ...
            <div class="brutal-card overflow-hidden bg-white">
                <div class="grid lg:grid-cols-5">
                    <div class="lg:col-span-2">
                        <img :src="highlightEvent.image" :alt="highlightEvent.title" class="h-full w-full object-cover" style="min-height: 280px;" />
                    </div>
                    <div class="flex flex-col justify-center p-8 lg:col-span-3 lg:p-10">
                        <div class="mb-3.5 inline-flex w-fit items-center gap-1.5 rounded-lg border-[1.5px] border-[var(--brutal-ink)] bg-[var(--brutal-mint)]/15 px-3 py-1 shadow-[var(--brutal-shadow-sm)]">
                            ...
                            <span class="text-[10px] font-bold text-[var(--brutal-ink)]">{{ highlightEvent.status }}</span>
                        </div>
                        <h3 class="font-display text-2xl font-bold tracking-[-0.035em] text-[var(--brutal-ink)] sm:text-3xl">{{ highlightEvent.title }}</h3>
                        <p class="mt-3 max-w-xl text-sm font-medium leading-relaxed text-[var(--brutal-ink)]/60">{{ highlightEvent.desc }}</p>
                        <div class="mt-6 flex flex-wrap items-center gap-6 text-xs font-bold text-[#34343B]">
                            ...
                            <span class="flex items-center gap-1.5">
                                ...
                                {{ highlightEvent.attendees.toLocaleString() }} attendees
                            </span>
                        </div>
                        <a href="/auth" class="mt-7 inline-flex w-fit items-center gap-2 rounded-xl border-[1.5px] border-[var(--brutal-ink)] bg-[var(--brutal-blue)] px-6 py-3 text-sm font-bold text-white shadow-[var(--brutal-shadow)] transition-all duration-200 hover:-translate-y-0.5 hover:shadow-[4px_4px_0_var(--brutal-ink)] active:translate-y-0 active:shadow-[1px_1px_0_var(--brutal-ink)]">
                            Register Now
